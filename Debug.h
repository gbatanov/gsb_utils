...
// в маке пишет в сислог, но посмотреть можно только командой в консоли: (gsblog - им€ программы)
// log show --info --debug --predicate "process == 'gsb'"
// log show --info --debug  --process gsb --last 10m

// Ќ≈ забывать при старте программы в дебаговом режиме запускать поток
...
// до версии 0.8.27 вс€ инициализаци€ на стороне вызывающей программы
...
public:
	...
	/// @brief
	/// @param level 0 отключает вс€кий отладочный вывод куда-либо
	...
	static void dprintf(int level, std::string fmt, ...)
	{
		...
		if (level <= debug_level)
		{
			...
			{
				tm tm;
				time_t t1 = time(nullptr); // текущее врем€ timestamp
